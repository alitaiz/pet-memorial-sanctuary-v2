...
export const MemorialCard = ({ memorial, onDelete, isOwner }: MemorialCardProps) => (
  <div className="bg-white/70 backdrop-blur-sm p-4 rounded-xl shadow-md flex items-center justify-between transition-transform duration-300 hover:scale-105 hover:shadow-lg">
    <div className="flex items-center space-x-4">
      <div className={`rounded-full p-2 ${isOwner ? 'bg-pink-100' : 'bg-gray-100'}`}>
        <HeartIcon className={`w-6 h-6 ${isOwner ? 'text-pink-500' : 'text-slate-400'}`} />
      </div>
      <div>
    ...
    <div className="flex items-center space-x-2">
      ...
      <button 
        onClick={() => onDelete(memorial.slug)} 
        className="text-red-400 hover:text-red-600 transition-colors p-1"
        title={isOwner ? "Permanently delete this memorial" : "Remove from my list"}
      >
